...
export async function POST() {
  ...
  const user = await prisma.user.findUnique({
    ...
    select: { subscriptionTier: true, creemSubscriptionId: true, subscriptionExpiryDate: true },
  })
  ...
  // 清除 Creem subscription ID（已取消），但保留 PRO 和到期时间
  // 用户付了钱，当前周期内仍享受 Pro 功能
  // dashboard API 的 checkSubscription 会在到期后自动降级
