...
export default function LoginClient({ isLoggedIn, userName, isDemo }: { isLoggedIn?: boolean; userName?: string; isDemo?: boolean }) {
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-bg-brand to-[#2a1c12] flex items-center justify-center p-4">
      <div className="bg-card rounded-2xl shadow-2xl p-8 max-w-md w-full ring-1 ring-orange-100/50">
        ...
        {isLoggedIn && (
          <div className="mb-4 p-4 bg-amber-500/10 border border-amber-500/30 rounded-xl">
            ...
            <p className="text-xs text-amber-500/70 mt-1">{t('switchAccount')}</p>
        ...
        {showOauthError && (
          <div className="relative mb-6 p-3 pr-8 rounded-xl bg-error/10 border border-error/30 text-xs text-error leading-relaxed">
            <button
              type="button"
        ...
        {tab === "email" && (
          <div className="space-y-4">
            ...
            {emailCodeSent && (
              <>
                ...
                <button
                  ...
                  className="w-full bg-accent text-white rounded-xl py-3 font-medium hover:bg-orange-600 disabled:bg-surface disabled:text-text-secondary transition-all active:scale-[0.98]"
                >
                  {loading === "email_login" ? t('loggingIn') : t('loginRegisterAction')}
                </button>
                {emailMsg && (
                  <div className="text-xs text-success bg-success/10 border border-success/30 rounded-xl px-3 py-2">
                    {emailMsg}
                  </div>
                )}
              </>
            ...
            {!emailCodeSent && error && tab === "email" && (
              <div className="text-xs rounded-xl px-3 py-2 bg-error/10 border-error/30 text-error">
                {error}
              </div>
            )}
            {!emailCodeSent && (
