...
/** 组装支付宝 API 请求参数 + 签名，返回 URL 编码的请求体 */
      ...
      async request(
        ...
        if (alipayPublicKey && data.sign) {
          const sorted = Object.keys(data.alipay_user_info_share_response)
            .sort()
            .map((k) => `${k}=${(data.alipay_user_info_share_response as Record<string, unknown>)[k]}`)
            .join("&")
