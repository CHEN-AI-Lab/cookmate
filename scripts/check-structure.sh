...
# [2/6] apps/*/src/ 下纯工具文件（无 next/* import, 无 'use client'）
echo ""
echo "[2/6] 检查移位的纯工具文件"
MOVED=""
...
# [5/6] 每个 app 的 package.json 引用 shared workspace
echo ""
echo "[5/6] Workspace 依赖检查"
for d in apps/*/; do
  name=$(basename "$d")
  if [ -f "$d/package.json" ]; then
    if grep -q '"@cookmate/shared"' "$d/package.json" 2>/dev/null; then
      echo "  ✅ apps/$name 引用了 @cookmate/shared"
    else
...
# [6/6] shared/ package.json exports 通配符
echo ""
