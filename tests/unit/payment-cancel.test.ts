// 取消订阅端点测试：Creem 渠道（Stripe 已删除，支付渠道 = Creem + 支付宝一次性付款）
...
describe('取消订阅', () => {
  ...
  it('FREE 用户 → 400', async () => {
    stores.users.set('u1', { id: 'u1', subscriptionTier: 'FREE', creemSubscriptionId: null })
    const res = await cancelPOST()
    expect(res.status).toBe(400)
  })
  it('Creem 订阅用户取消：调用 cancelSubscription 并清空 creemSubscriptionId（保留 PRO 到到期）', async () => {
    stores.users.set('u1', { id: 'u1', subscriptionTier: 'PRO', creemSubscriptionId: 'creem_sub_1', subscriptionExpiryDate: new Date(Date.now() + 86400000) })
    const res = await cancelPOST()
  ...
  it('取消 API 抛错 → 保留本地订阅ID（fail-closed：便于 webhook 到期降级 / 可重试取消）', async () => {
    ...
    stores.users.set('u1', { id: 'u1', subscriptionTier: 'PRO', creemSubscriptionId: 'creem_sub_1' })
    const res = await cancelPOST()
    // 取消失败 → 207 Multi-Status（让用户感知失败，可重试）
  ...
  // 场景：支付宝一次性付款 / webhook 失败遗留 / 错过回调
  it('PRO 用户但无 creem 订阅ID（遗留状态） → 409，提示无需取消', async () => {
    stores.users.set('u1', { id: 'u1', subscriptionTier: 'PRO', creemSubscriptionId: null, subscriptionExpiryDate: new Date(Date.now() + 86400000) })
